...
def afficher_ascii_htrack():
    """Affiche un dessin ASCII pour HTrack."""
    print(r"""
   _   _ _____ _____            _    
  | | | |_   _|_   _|          | |   
  | |_| | | |   | |  _ __   ___| | __
...
def voir_logs(log_file):
    ...
    if os.path.exists(log_file):
        ...
        with open(log_file, "r") as f:
            lines = f.readlines()
            for line in lines[-10:]:  # Derniers 10 logs
                print(line.strip())
    else:
        print("\nAucun log trouvé.\n")
...
def main():
    logger = setup_logger("web_cloner", "cloner.log")
    afficher_ascii_htrack()
    
    global default_output_dir, file_types
    default_output_dir = "output"  # Paramètre par défaut
    file_types = ["html", "css", "js", "png", "jpg", "jpeg", "gif"]  # Types par défaut
